...
const Pendencias = () => {
  ...
  const renderIcone = (tipo: string) => {
    switch (tipo) {
      case "erro":
        return <FileWarning size={24} className="text-red-500" />;
      case "alerta":
    ...
    <div className="max-w-5xl mx-auto px-4 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Análises que exigem sua intervenção para prosseguir.
          </p>
        </div>
      ...
      <div className="space-y-4">
        {pendencias.map((item) => (
          <div
            key={item.id}
            className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col md:flex-row md:items-center justify-between gap-6 group"
          >
            <div className="flex items-start space-x-4 flex-1">
              <div
                className={`p-3 rounded-xl flex-shrink-0 ${item.tipo === "erro" ? "bg-red-50" : item.tipo === "alerta" ? "bg-orange-50" : "bg-blue-50"}`}
              >
        ...
        {pendencias.length === 0 && (
          <div className="bg-slate-50 border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">
              Nenhuma pendência aguardando sua ação no momento.
            </p>
          </div>
        )}
      </div>
